...
try {
    ...
    if (!($AutoDownloadAzSHCIiso)) {
        if (!($AzureStackHCIIsoPath)) {
            Write-Host "`nHave you downloaded an Azure Stack HCI 23H2 ISO? If not, one will be downloaded automatically for you"
    ...
    Invoke-WebRequest -Uri 'https://github.com/DellGEOS/HybridJumpstart/raw/main/dsc/HybridJumpstart23H2.zip' `
